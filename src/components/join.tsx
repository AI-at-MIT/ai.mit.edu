...
function ApplyNow({initiative} : {initiative: Constants.InitiativeInterface}) {
    return (
      <div className="m-20">
      <a href="google.com" className={`${initiative.border_class} border-4 event-card group rounded-full border border-transparent w-96 h-32 px-5 py-4 flex items-center transition duration-100 active:opacity-50 active:scale-95`}>
...
export default function Join({initiative, heading, blurb_recruiting, blurb_notrecruiting, isrecruiting, link} : {initiative: Constants.InitiativeInterface, heading: string, blurb_recruiting: string[], blurb_notrecruiting: string[], isrecruiting: boolean, link: string}) {
    ...
    const button = isrecruiting ? (<ApplyNow initiative={initiative}/>) : (<JoinMailingList initiative={initiative}/>)
    return (
